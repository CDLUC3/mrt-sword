...
public class SwordAPIEndpoint
{
			...
			throws ServletException, IOException, SwordError
	{
		...
		MimeMultipart mimeMultipart = this.getPartsFromRequestMime(req);
                String slug = req.getHeader("Slug");
                deposit.setSlug(slug);
                int size = mimeMultipart.getCount();
                System.out.println("***size=" + size);
                for (int i = 0; i < size; i++) {
                    BodyPart item = null;
                    item = mimeMultipart.getBodyPart(i);
                    System.out.println("class:" + item.getClass().getName());
			// find out which part we are looking at
                        
			String[] contentDispositions = item.getHeader("Content-Disposition");
			...
			{
				String[] md5s = item.getHeader("Content-MD5");
                                String md5 = md5s[0];
				String[] packagings = item.getHeader("Packaging");
                                String packaging = packagings[0];
