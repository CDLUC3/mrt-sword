...
import org.swordapp.server.action.EditHandler;
...
public class SwordAPIEndpoint
{
	...
	protected void storeAndCheckBinaryMultipart(Deposit deposit, String receivedMD5, SwordConfiguration config)
		...
		if (deposit.getFile() == null)
		{
			throw new SwordServerException("Attempting to store and check deposit which has no input stream");
                ...
                System.out.println("***Tests:"
                        + " - config.getMaxUploadSize():" + config.getMaxUploadSize()
                        + " - fLength:" + fLength
                        + " - deposit.getMd5():" + deposit.getMd5()
			...
			throws ServletException, IOException, SwordError
	{
		...
		// Parse the request for files (using the fileupload commons library)
                EditHandler editHandler = new EditHandler(req);
                editHandler.parse();
			...
			String contentDisposition = editHandler.getHeader("Content-Disposition");
			String name = this.getContentDispositionValue(contentDisposition, "name");  
			if ("atom".equals(name))
			{
				throw new ServletException("atom multipart not supported");
			}
			else if ("payload".equals(name))
			{
				String md5 = editHandler.getHeader("Content-MD5");
				String packaging = editHandler.getHeader("Packaging");
				...
				String ct = editHandler.getHeader("Content-Type");
				...
				if (ct != null)
				{
					...
					mimeType = bits[0].trim();
                                ...
                                File zip = editHandler.getZipFile();
		...
		try
		{
                        String receivedMd5 = editHandler.getMd5();
			this.storeAndCheckBinaryMultipart(deposit, receivedMd5, this.config);
		}
	...
	protected Collection<Part> getPartsFromRequest(HttpServletRequest request)
	...
	protected MimeMultipart getPartsFromRequestMime(HttpServletRequest request)
			throws ServletException
	{
		try
		{
